...
public class FrameGenerator {
	...
	public FrameGenerator(String type,String id) {
		this.id = id;
		this.type = type;
	...
	public FrameInfo[] getFrameInfoArray() {
		return frameInfoArray;
	}
	
	public Comparator<Interval> cmp = new Comparator<Interval>() {
		@Override
...
//		String endName = "time_end";
	...
	private Interval[] getIntervalArray() {
		List<Interval> list = removeDuplicateStart();
		...
		while (i < list.size()) {
			...
			for (long j = curS; j < curE; j += 3600) {
				...
				res[curP++] = new Interval(j, j + 3600, curId, curS);
			}
			i++;
		}
	...
	private FrameInfo[] getFrameInfoList() {
		if(null != type && type.equals("pul_lis")){
			...
			if (null != curList && curList.size() == 1) {
				...
				this.movieEnd = (FrameGenerator.t - 8) * 60 * 60;
				Map<String, String> resultMap = (Map<String, String>) curList.get(0);
				FrameInfo[] res = new FrameInfo[FrameGenerator.t];
				for(int i = 0; i < FrameGenerator.t; i++){
					String fName = resultMap.get("ncfilename").substring(1, resultMap.get("ncfilename").length() - 1);
					res[i] = new FrameInfo(fName, i);
				}
				return res;
			}
			return null;
		}else{
			Interval[] intervals = getIntervalArray();
			if(null == intervals)
			{
				return null;
			}
			int n = intervals.length;
			FrameInfo[] res = new FrameInfo[n];
			int i = 0;
			while (i < n) {
				if (intervals[i] != null) {
					...
					res[i] = new FrameInfo(ncfilename, time);
				}
				i++;
			}
			return res;
